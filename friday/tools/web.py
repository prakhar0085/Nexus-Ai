...
def register(mcp):
    ...
    async def search_web(query: str) -> str:
        ...
        async with httpx.AsyncClient(follow_redirects=True, timeout=10) as client:
            ...
            if response.status_code != 200:
                return f"Web search failed. Status code: {response.status_code}"
            
            # Extract snippets via regex to avoid heavy HTML parsers
            snippets = re.findall(r'<a class="result__snippet[^>]*>(.*?)</a>', response.text, re.IGNORECASE | re.DOTALL)
            if not snippets:
                return "No useful search results found on the web."
                
            results = []
            for s in snippets[:4]: # Grab top 4 results
                clean_text = re.sub('<[^<]+?>', '', s)
